...
namespace VURO.UI.Controllers
{
    public class EditionsController : Controller
    {
        ...
        {
            var model = _repository.Articles.Where(x => x.EditionId == idedition)
                .Include(x => x.DataArticleAuthors)
                .ThenInclude(x => x.Author).OrderBy(x => x.ArtPos).ToList();
        ...
        public IActionResult LastArticles()
        {
            try
            {
                ...
                var model = _repository.Articles.Where(x => x.EditionId == LastEdition.Id)
                    .Include(x => x.DataArticleAuthors).ThenInclude(x => x.Author).OrderBy(x => x.ArtPos);
