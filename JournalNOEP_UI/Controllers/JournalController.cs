...
namespace JournalNOEP_UI.Controllers
{
    ...
    public class JournalController : Controller
    {
        ...
        public async Task<IActionResult> EditEditionFile(DataEdition dedition, IFormFile EditionFileNew)
        {
            ...
            int AdditionalEdition = _edition.AdditionalEdition ?? 0;
            // путь к папке Files
            string fileName = _edition.IdYearNavigation.Year + "_" + _edition.Edition + "_" + AdditionalEdition + Path.GetExtension(EditionFileNew.FileName);
        ...
        public IActionResult Edition(Guid? idedition)
        {
           ...
           // var model = _repository.Articles.Include(z => z.Edition).ThenInclude(c => c.IdYearNavigation).Where(x => x.EditionId == idedition).Include(a => a.DataArticleAuthors).ThenInclude(b => b.Author).OrderBy(x => x.ArtPos);
            ...
            ViewBag.DateLastArticle = edition.DateLastArticle;
        ...
        public IActionResult PartialEditArticleFileXml(Guid Id)
        {
            var model = _repository.Articles.FirstOrDefault(x => x.Id == Id);
            return PartialView("Edition/PartialEditArticleFileXml", model);
        }
        [HttpPost]
        public async Task<IActionResult> EditArticleFileXml(Articles articless, IFormFile ArtFileNew)
        {

            var articles = _repository.Articles.FirstOrDefault(x => x.Id == articless.Id);
            // путь к папке Files
            string fileName = Guid.NewGuid() + Path.GetExtension(ArtFileNew.FileName);
            string path = Path.Combine("/Files", "XmlFiles", fileName);
            using (var fileStream = new FileStream(_appEnvironment.WebRootPath + path, FileMode.Create))
            {
                await ArtFileNew.CopyToAsync(fileStream);
            }
            articles.ArtFileXml = fileName;
            _repository.Update(articles);
            return RedirectToAction("Index", new { idedition = articles.EditionId });
        }
        ...
        [HttpPost]
        public async Task<IActionResult> EditArticle(Articles articles, IFormFile ArtFileNew, Guid[] ArtAuthors, IFormFile ArtFileNewXML)
        {
            articles.ArtFileXml = ".";
            if (ArtFileNew != null)
            {
                
                if (ArtFileNewXML != null)
                {
                    articles.ArtFileXml = Guid.NewGuid() + Path.GetExtension(ArtFileNewXML.FileName);
                    string pathxml = Path.Combine("/Files", "XmlFiles", articles.ArtFileXml);
                    using (var fileStream = new FileStream(_appEnvironment.WebRootPath + pathxml, FileMode.Create))
                    {
                        await ArtFileNew.CopyToAsync(fileStream);
                    }
                }
                ...
                articles.ArtPos = _repository.Articles.Max(x => x.ArtPos) + 1;
